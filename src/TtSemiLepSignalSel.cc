...
{ 
  ...
  math::XYZTLorentzVector tj = topJets[0].p4();  
  var_dRlepjet1 = TMath::Sqrt((tj.Eta()-lepton.Eta())*(tj.Eta()-lepton.Eta())
	    		        +(tj.Phi()-lepton.Phi())*(tj.Phi()-lepton.Phi()));

  //var_etajet1 = fabs(tj.Eta());

  
  var_dphilepjet1 = fabs(tj.Phi()-lepton.Phi());
  if (var_dphilepjet1 > 3.1415927)  var_dphilepjet1 =  2*3.1415927 - var_dphilepjet1;
  if (var_dphilepjet1 < -3.1415927) var_dphilepjet1 = -2*3.1415927 - var_dphilepjet1;
